...
##Pokemon sprint 3 final project
    charzard_frame = ttk.Frame(main_frame,padding = 10, borderwidth = 5, relief = 'ridge')
    charzard_frame.grid(row = 0, column = 1)

    title = ttk.Label(charzard_frame, text = 'attacks')
    title.grid(row = 1, column = 2)

    growl= ttk.Button(charzard_frame, text = 'growl')
    growl.grid(row = 2, column = 1)
    growl['command'] = lambda: handle_growl(mqtt_sender)

    quick_attack = ttk.Button(charzard_frame, text = 'quick attack')
    quick_attack.grid(row = 2, column = 3)
    quick_attack['command'] = lambda: handle_quick_attack(mqtt_sender)

    take_down = ttk.Button(charzard_frame, text = 'take down')
    take_down.grid(row = 3, column = 1)
    take_down['command'] = lambda: handle_take_down(mqtt_sender)

    seismic = ttk.Button(charzard_frame, text = 'seismic slam')
    seismic.grid(row = 3, column = 3)
    seismic['command'] = lambda: handle_seismic(mqtt_sender)


    def handle_growl(mqtt_sender):
        print('charzard use growl!!')
        mqtt_sender.send_message('growl')

    def handle_quick_attack(mqtt_sender):
        print('charzard use quick attack!!')
        mqtt_sender.send_message('quick_attack')

    def handle_take_down(mqtt_sender):
        print('charzard use tackle!!')
        mqtt_sender.send_message('take_down')

    def handle_seismic(mqtt_sender):
        print('charzard use seismic smash!!')
        mqtt_sender.send_message('seismic')
